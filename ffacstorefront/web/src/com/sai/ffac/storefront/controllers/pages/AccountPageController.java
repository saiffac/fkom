...
public class AccountPageController extends AbstractSearchPageController
{
	...
	@RequestMapping(value = "/update-sap-code", method = RequestMethod.GET)
	@RequireHardLogIn
	public String editSapCode(final Model model) throws CMSItemNotFoundException
	{
		final FfacCustomerData customerData = customerFacade.getCurrentCustomer();
		final UpdateSapCodeForm updateSapCodeForm = new UpdateSapCodeForm();
		updateSapCodeForm.setSapCode(customerData.getSapCode());
		model.addAttribute("updateSapCodeForm", updateSapCodeForm);

		storeCmsPageInModel(model, getContentPageForLabelOrId(UPDATE_SAP_CODE_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(UPDATE_SAP_CODE_CMS_PAGE));
	...
	@RequestMapping(value = "/update-sap-code", method = RequestMethod.POST)
	@RequireHardLogIn
	public String updateSapCode(final UpdateSapCodeForm updateSapCodeForm, final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		//override the customerData facade to avoid creating a new one --> should separate in to different facade for this case

		getSapCodeValidator().validate(updateSapCodeForm, bindingResult);
		if (!bindingResult.hasErrors())
		{
			//			String returnAction = ControllerConstants.Views.Pages.Account.AccountProfileSapCodePage;
			...
			customerData.setSapCode(updateSapCodeForm.getSapCode());
			...
			{
				bindingResult.rejectValue("sapCode", "profile.sapCode.invalid"); //redundant because of changing nothing with uId
			}
		}
		...
		if (bindingResult.hasErrors())
		{
			...
			storeCmsPageInModel(model, getContentPageForLabelOrId(UPDATE_SAP_CODE_CMS_PAGE));
			setUpMetaDataForContentPage(model, getContentPageForLabelOrId(UPDATE_SAP_CODE_CMS_PAGE));
			model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.profile.updateSapCode"));
			return getViewForPage(model);
		}
		else
		{
			GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER,
					"text.account.profile.sapCodeUpdated", null);
			return REDIRECT_TO_PROFILE_PAGE;
		}
	}
